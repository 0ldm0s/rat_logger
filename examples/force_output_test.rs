use rat_logger::{LoggerBuilder, LevelFilter, Record, Metadata, emergency, flush_logs};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    LoggerBuilder::new()
        ...
        .with_batch_config(rat_logger::BatchConfig {
    ...
    println!("2. 测试强制输出命令（预期：立即输出）：");
    emergency!("这是强制输出的紧急日志1 - 应该立即输出");
    emergency!("这是强制输出的紧急日志2 - 应该立即输出");
    ...
    info!("普通日志 A - 被缓冲");
    info!("普通日志 B - 被缓冲");
    emergency!("紧急日志 C - 强制输出（同时会刷新缓冲区的A和B）");
    info!("普通日志 D - 被缓冲");
