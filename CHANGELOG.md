...
### 新增
- **UDP处理器架构重构**: 重构UDP处理器与终端处理器保持一致的架构模式
- **FileConfig同步模式支持**: 添加force_sync字段，支持强制同步写入模式
- **性能优化**: 修复异步模式下计时器线程缺失问题，提升日志系统性能
- **批处理优化**: 修复工作线程Shutdown命令过早发送问题，确保日志完整性

### 修复
- **时序问题**: 修复强制输出测试的时序问题，提高日志处理的可靠性
- **资源清理**: 改进同步模式下的资源处理和清理机制
- **编译兼容性**: 修复zerg_overmind和test_client项目中的API变更兼容性问题

### 改进
- **架构一致性**: 统一UDP处理器和终端处理器的批量处理控制逻辑
- **API优化**: 将add_terminal()方法改为add_terminal_with_config()，提供更好的配置控制
- **错误处理**: 改进UDP处理器的错误处理和状态管理机制

### 技术细节
- 优化了批处理逻辑，确保异步模式下的性能和可靠性
- 改进了FileProcessor和TermProcessor的架构一致性
- 添加了强制同步模式支持，提高关键场景的日志可靠性
