...
rat_loggerは、Rustで書かれた高性能でスレッドセーフなロギングライブラリです。プロデューサー-コンシューマーアーキテクチャと非同期書き込み機構を採用し、優れたパフォーマンスと柔軟な設定オプションを提供します。

## 特徴

- **究極のパフォーマンス**: プロデューサー-コンシューマーアーキテクチャにより、MacBook Air M1環境で実測ファイル書き込み性能40万+ msg/sec（参考値）
...
#### UDPネットワーク転送パフォーマンス
- 100メッセージバッチ: ~725,516 メッセージ/秒
- 1000メッセージバッチ: ~860,739 メッセージ/秒
- 5000メッセージバッチ: ~921,326 メッセージ/秒

*注：UDPネットワーク転送テストはローカルループバックインターフェース（127.0.0.1）で実施。実際のネットワーク環境ではパフォーマンスが異なる場合があります*
